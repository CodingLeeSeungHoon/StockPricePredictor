...
class Stopwords:
    ...
    def _get_replace_stopwords(self, title_token_list):
        """
        title_str을 읽어 들여서
        ...
        replace_dict = {'比': '대비', '↓': '감소', '↑': '증가', 'Q': '분기', '中': '중국', '日': '일본', '美': '미국', '韓': '한국',
                            '北': '북한', '$': '달러', '￥': '엔', '€': '유로'}
        replace_table = title_token_list.maketrans(replace_dict)
        title_token_list = re.sub('[0-9]', '', title_token_list)
        title_token_list = title_token_list.translate(replace_table)
        return title_token_list
